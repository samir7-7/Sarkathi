...
<% Integer adminId=(Integer)request.getAttribute("adminId");String adminName=(String)request.getAttribute("adminName");if(adminName==null)adminName="Admin"; %>
...
function renderApps(apps){
    ...
    apps.forEach(a=>{
        tbody.innerHTML+='<tr class="hover:bg-slate-50 transition">'
            ...
            +'<td class="px-6 py-4"><button onclick=\'openReview('+JSON.stringify(a).replace(/\'/g,"\\&#39;")+')\' class="rounded-lg bg-brand-50 px-3 py-1.5 text-xs font-semibold text-brand-900 hover:bg-brand-100 transition">Review</button>'
...
function openReview(app){
    ...
    document.getElementById('modal-docs').innerHTML=docs.length===0
        ...
        :docs.map(d=>'<a class="flex items-center justify-between rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-700 hover:bg-slate-50" href="'+CTX+'/'+d.filePath+'" target="_blank"><span>'+d.documentType+'</span><span class="text-xs text-brand-900 font-semibold">Open</span></a>').join('');
