...
<header class="hidden lg:block px-6 py-6 lg:px-12 relative z-10 transition-all duration-300">
    <nav class="mx-auto grid max-w-7xl grid-cols-[1fr_auto_1fr] items-center gap-6">
        ...
        <div class="flex items-center justify-center gap-8 text-[11px] font-black uppercase tracking-[0.2em] text-slate-500">
            <% 
            ...
            <a href="<%= request.getContextPath() %>/budget" class="<%= currentPathTop.contains("budget") ? linkActiveClass : linkBaseClass %>">Budget</a>
        ...
        <div class="flex items-center justify-end gap-4">
            ...
            <% } else { %>
                <a href="<%= request.getContextPath() %>/login" class="text-[11px] font-black uppercase tracking-[0.2em] text-brand-900 hover:text-brand-500 transition-colors">Login</a>
                <a href="<%= request.getContextPath() %>/register" class="rounded-xl bg-brand-900 px-8 py-3.5 text-[11px] font-black uppercase tracking-[0.2em] text-white shadow-xl shadow-brand-900/20 hover:bg-slate-900 transition-all active:scale-95">Register</a>
            <% } %>
        </div>
    </nav>
</header>
