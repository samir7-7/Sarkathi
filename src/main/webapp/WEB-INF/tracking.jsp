...
<aside id="sidebar" class="fixed inset-y-0 left-0 z-30 flex w-[220px] -translate-x-full flex-col border-r border-slate-200 bg-white transition-transform duration-300 lg:static lg:flex lg:translate-x-0">
    <div class="flex items-center justify-between px-5 pt-5 pb-2">
        ...
        <button onclick="toggleSidebar()" class="p-2 text-slate-600 lg:hidden focus:outline-none">
    ...
    <div class="mx-5 mt-3 flex items-center gap-3 rounded-xl bg-brand-50 px-4 py-3">
        ...
        <div class="min-w-0"><p class="text-sm font-semibold text-brand-900 truncate"><%= citizenName %></p><p class="text-[11px] text-slate-500">Citizen</p></div>
    </div>
...
<div class="flex-1 flex flex-col min-h-screen">
    ...
    <% } else { %>
    <header class="sticky top-0 z-20 flex items-center justify-between border-b border-slate-200 bg-white/80 backdrop-blur-md px-4 py-3.5 lg:px-8">
        <div class="flex items-center gap-4">
            <button onclick="toggleSidebar()" class="p-2 text-slate-600 lg:hidden focus:outline-none">
                <i data-lucide="menu" class="h-6 w-6"></i>
            </button>
            <div><h1 class="text-lg font-bold text-slate-900">Track Application</h1><p class="text-xs text-slate-500 hidden sm:block">Check the status of your applications</p></div>
        </div>
        ...
        <div class="w-full fade-in">
            <div class="mb-8 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
                ...
                <div class="flex flex-col gap-3 md:flex-row">
                    <input id="tracking-input" type="text" placeholder="Enter Tracking ID" class="flex-1 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-sm">
                        <thead><tr class="border-t border-b border-slate-100">
                            ...
                            <th class="px-6 py-3 text-[11px] font-semibold uppercase tracking-wider text-slate-400">Submitted At</th>
                            <th class="px-6 py-3 text-[11px] font-semibold uppercase tracking-wider text-slate-400">Last Updated</th>
                            <th class="px-6 py-3 text-[11px] font-semibold uppercase tracking-wider text-slate-400">Status</th>
                            <th class="px-6 py-3 text-[11px] font-semibold uppercase tracking-wider text-slate-400">Remarks</th>
                        </tr></thead>
...
function searchTracking(){
    ...
    fetch(CTX+'/api/applications?trackingId='+encodeURIComponent(tid)).then(r=>r.json()).then(data=>{
        ...
        if(!app){container.innerHTML='<div class="rounded-2xl border border-red-100 bg-red-50 p-6 text-center"><p class="text-sm text-red-600 font-semibold">No application found</p></div>';return;}
        container.innerHTML='<div class="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm"><div class="flex items-center justify-between mb-6"><h3 class="text-lg font-bold text-slate-900">#'+app.trackingId+'</h3>'+badge(app.status)+'</div>'
            +'<div class="grid grid-cols-2 gap-4 text-sm"><div><p class="text-[10px] uppercase text-slate-400">Submitted</p><p class="font-medium text-slate-700">'+fmtDate(app.submittedAt)+'</p></div>'
            +'<div><p class="text-[10px] uppercase text-slate-400">Service</p><p class="font-medium text-slate-700">'+(app.serviceName||app.serviceTypeId)+'</p></div></div></div>';
    }).catch(()=>{container.innerHTML='<p class="text-center py-4 text-red-400">Error</p>';});
}
<% if(loggedIn){ %>
fetch(CTX+'/api/applications?citizenId=<%= citizenId %>').then(r=>r.json()).then(apps=>{
    const tbody=document.getElementById('apps-body');
    if(!Array.isArray(apps)||apps.length===0){tbody.innerHTML='<tr><td colspan="6" class="px-6 py-8 text-center text-sm text-slate-400">No applications</td></tr>';return;}
    tbody.innerHTML='';
    apps.forEach(a=>{
        ...
        tr.innerHTML='<td class="px-6 py-4 font-semibold text-brand-900">#'+a.trackingId+'</td>'
            +'<td class="px-6 py-4 text-slate-600">'+(a.serviceName||a.serviceTypeId)+'</td>'
            +'<td class="px-6 py-4 text-slate-500">'+fmtDate(a.submittedAt)+'</td>'
            +'<td class="px-6 py-4 text-slate-500">'+fmtDate(a.lastUpdatedAt)+'</td>'
            +'<td class="px-6 py-4">'+badge(a.status)+'</td>'
            +'<td class="px-6 py-4 text-slate-500 max-w-[200px] truncate">'+(a.remarks||'—')+'</td>';
