...
<% Integer citizenId=(Integer)request.getAttribute("citizenId"); String citizenName=(String)request.getAttribute("citizenName"); String pageError=(String)request.getAttribute("pageError"); String formError=request.getParameter("error"); Integer unread=(Integer)request.getAttribute("unreadCount"); List<ServiceType> serviceTypes=(List<ServiceType>)request.getAttribute("serviceTypes"); List<Ward> wards=(List<Ward>)request.getAttribute("wards"); List<CitizenDocumentVault> documents=(List<CitizenDocumentVault>)request.getAttribute("documents"); if(citizenName==null)citizenName="Citizen"; if(unread==null)unread=0; if(serviceTypes==null)serviceTypes=List.of(); if(wards==null)wards=List.of(); if(documents==null)documents=List.of(); String initials=citizenName.isBlank()?"C":citizenName.substring(0,1).toUpperCase(); %>
<!DOCTYPE html>
<html lang="en">
    <head>
        <meta charset="UTF-8">
        ...
        <div class="flex min-h-screen">
            <aside class="fixed inset-y-0 left-0 z-30 flex w-[220px] flex-col border-r border-slate-200 bg-white">
                ...
                <nav class="mt-6 flex-1 space-y-1 px-3">
                    ...
                    <a href="<%= request.getContextPath() %>/citizen/apply" class="sidebar-link active flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm">
            ...
            <div class="ml-[220px] flex-1 flex flex-col min-h-screen">
                <header class="sticky top-0 z-20 flex items-center justify-between border-b border-slate-200 bg-white/80 px-8 py-3.5">
                    <div>
                        ...
                        <p class="text-xs text-slate-500">
                            Submit a new application for certificates or registrations
                        </p>
                    </div>
                ...
                <main class="flex-1 px-8 py-8 overflow-y-auto">
                    ...
                    <% } %>
                    <form method="post" action="<%= request.getContextPath() %>/api/applications" class="mx-auto max-w-3xl space-y-6">
                        ...
                        <section class="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
                            ...
                            <div class="mt-5 grid gap-4 md:grid-cols-2">
                                <div>
                                    ...
                                    <select name="serviceTypeId" required class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:border-brand-500">
                                        <% for(ServiceType s: serviceTypes){ %>
                                            <option value="<%= s.getServiceTypeId() %>">
                                                <%= esc(s.getServiceName()) %>
                                                - Rs.
                                                <%= esc(s.getBaseFee()) %>
                                            </option>
                                        <% } %>
                                ...
                                <div>
                                    ...
                                    <select name="wardId" required class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:border-brand-500">
                                        <% for(Ward w: wards){ %>
                                            <option value="<%= w.getWardId() %>">
                                                Ward
                                                <%= w.getWardNumber() %>
                                                -
                                                <%= esc(w.getMunicipalityName()) %>
                                            </option>
                                        <% } %>
                                    </select>
                                </div>
                            ...
                            <div class="mt-4">
                                ...
                                <textarea name="formData" rows="6" required placeholder="Write the required details for this service." class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:border-brand-500">
                                </textarea>
                            </div>
                        </section>
                        <section class="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
                            <h2 class="text-lg font-bold text-slate-900">
                                Reuse Saved Documents
                            </h2>
                            <% if(documents.isEmpty()){ %>
                                <p class="mt-3 text-sm text-slate-500">
                                    No saved documents yet. Upload reusable documents from My Documents first.
                                </p>
                            <% } else { %>
                                <div class="mt-4 grid gap-3 md:grid-cols-2">
                                    <% for(CitizenDocumentVault d: documents){ %>
                                        <label class="flex items-center gap-3 rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-700">
                                            <input type="checkbox" name="reuseDocumentIds" value="<%= d.getVaultDocId() %>">
                                            <span>
                        ...
                        <button type="submit" class="w-full rounded-xl bg-brand-900 py-3 text-sm font-semibold text-white hover:bg-brand-800">
                            Submit Application
                        </button>
                    </form>
                </main>
